...
(() => {
  ...
  // Scroll-linked word focus and sequential card reveals
  const revealObserver = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (!entry.isIntersecting) return;
      entry.target.classList.add(entry.target.matches('[data-stagger-list]') ? 'is-visible' : 'is-sharp');
      revealObserver.unobserve(entry.target);
    });
  }, { rootMargin: '0px 0px -16% 0px', threshold: 0.18 });

  document.querySelectorAll('[data-scroll-heading], [data-stagger-list]').forEach((element) => revealObserver.observe(element));
